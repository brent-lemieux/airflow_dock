...
copy_staging_events_task = PostgresOperator(
    task_id="copy_staging_events_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.staging_events_copy.format(
        CREDENTIALS.access_key, CREDENTIALS.secret_key
    )
)

copy_staging_songs_task = PostgresOperator(
    task_id="copy_staging_songs_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.staging_songs_copy.format(
        CREDENTIALS.access_key, CREDENTIALS.secret_key
    )
)

insert_songplay_task = PostgresOperator(
    task_id="insert_songplay_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.songplay_table_insert
)

insert_user_task = PostgresOperator(
    task_id="insert_user_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.user_table_insert
)

insert_song_task = PostgresOperator(
    task_id="insert_song_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.song_table_insert
)

insert_artist_task = PostgresOperator(
    task_id="insert_artist_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.artist_table_insert
)

insert_time_task = PostgresOperator(
    task_id="insert_time_task",
    dag=dag,
    postgres_conn_id="aws_redjam_cluster",
    sql=sql_initial.time_table_insert
)

create_infrastructure_task >> create_staging_events_task
create_infrastructure_task >> create_staging_songs_task

create_staging_events_task >> copy_staging_events_task
create_staging_songs_task >> copy_staging_songs_task

copy_staging_events_task >> create_songplay_task
copy_staging_songs_task >> create_songplay_task
create_songplay_task >> insert_songplay_task

copy_staging_events_task >> create_user_task
create_user_task >> insert_user_task

copy_staging_songs_task >> create_song_task
create_song_task >> insert_song_task

copy_staging_songs_task >> create_artist_task
create_artist_task >> insert_artist_task

insert_songplay_task >> create_time_task
create_time_task >> insert_time_task
